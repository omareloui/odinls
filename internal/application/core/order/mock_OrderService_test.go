...
// GetCurrentMerchantOrders provides a mock function with given fields: claims, opts
func (_m *MockOrderService) GetCurrentMerchantOrders(claims *jwtadapter.JwtAccessClaims, opts ...order.RetrieveOptsFunc) ([]order.Order, error) {
	_va := make([]interface{}, len(opts))
	for _i := range opts {
		_va[_i] = opts[_i]
	}
	...
	if len(ret) == 0 {
		panic("no return value specified for GetCurrentMerchantOrders")
	}
