...
// GetOrdersByMerchantID provides a mock function with given fields: id, opts
func (_m *MockOrderRepository) GetOrdersByMerchantID(id string, opts ...order.RetrieveOptsFunc) ([]order.Order, error) {
	_va := make([]interface{}, len(opts))
	for _i := range opts {
		_va[_i] = opts[_i]
	}
	...
	if len(ret) == 0 {
		panic("no return value specified for GetOrdersByMerchantID")
	}
